...
// Get current time in Karachi timezone as HH:MM format
const getCurrentIslamabadTime = () => {
  const date = new Date();
...
// Get current date in Karachi timezone as YYYY-MM-DD format
const getCurrentIslamabadDate = () => {
  const date = new Date();
  const parts = new Intl.DateTimeFormat("en-GB", {
    day: "2-digit",
    month: "2-digit",
...
// Get current weekday in Karachi timezone
const getCurrentIslamabadWeekday = () => {
  const todayValue = getCurrentIslamabadDate();
  const date = new Date(`${todayValue}T00:00:00+05:00`);
...
const validateSlot = (slot) => {
  ...
  if (
    startMinutes % APPOINTMENT_MINUTES !== 0 ||
    endMinutes % APPOINTMENT_MINUTES !== 0
  ) {
  ...
  // Validate that slots for today cannot be in the past
  const todayWeekday = getCurrentIslamabadWeekday();
  const currentTime = getCurrentIslamabadTime();
  const currentMinutes = timeToMinutes(currentTime);

  if (slot.day_of_week === todayWeekday && startMinutes <= currentMinutes) {
...
const setAvailability = async (req, res) => {
  try {
    ...
    const totalsByDay = slots.reduce((totals, slot) => {
      const startMinutes = timeToMinutes(slot.start_time);
      const endMinutes = timeToMinutes(slot.end_time);
      totals[slot.day_of_week] =
        (totals[slot.day_of_week] || 0) + (endMinutes - startMinutes);
      return totals;
    }, {});
    ...
    for (const [day, totalMinutes] of Object.entries(totalsByDay)) {
      if (totalMinutes > MAX_DUTY_MINUTES_PER_DAY) {
        return res
          .status(400)
          .json({ error: `Duty time for ${day} cannot exceed 8 hours` });
      }
    }
    ...
    for (let index = 1; index < sortedSlots.length; index += 1) {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "Availability slots cannot overlap" });
      }
    }

    const savedSlots = await Availability.setDoctorAvailability(
      doctorId,
      slots,
    );
    res.status(201).json({
...
const addSlot = async (req, res) => {
  try {
    ...
    const currentDutyMinutes = await Availability.getTotalDutyMinutesForDay(
      doctorId,
      day_of_week,
    );
    ...
    if (currentDutyMinutes + requestedMinutes > MAX_DUTY_MINUTES_PER_DAY) {
      return res
        .status(400)
        .json({ error: "A doctor can set at most 8 hours of duty per day" });
    }

    console.log(
      `Adding slot for doctor ${doctorId}: ${day_of_week} ${start_time}-${end_time}`,
    );

    const slot = await Availability.addAvailabilitySlot(
      doctorId,
      day_of_week,
      start_time,
      end_time,
